...
export const useUpdateTask = () => {
  return useMutation({
    mutationFn: async ({ task }) => {
      const { id, photoFile, ...updateData } = task;
      let photoUrl = updateData.photoUrl || "";
      // If a new photoFile is provided, upload and update photoUrl
      if (photoFile) {
      ...
      await updateDoc(docRef, { ...updateData, photoUrl });
    },
    onSuccess: () => {
...
export const addTask = async (task) => {
  ...
  if (task.photoFile) {
    ...
    photoUrl = "https://firebasestorage.googleapis.com/v0/b/stantec-dpg.firebasestorage.app/o/tasks%2F1757309300737-bccp9g8n.png?alt=media"
  }
